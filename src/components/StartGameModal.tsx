    ...
    <div className="modal-overlay">
      <div className="glass-panel w-full max-w-lg p-6 sm:p-8 rounded-3xl border-white/15 relative space-y-5">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            <h2 className="text-2xl font-extrabold text-white font-['Outfit'] flex items-center gap-2">
              <Swords className="text-[#00FF87]" size={22} /> Start A Game
            </h2>
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">Golf Course</label>
          {course ? (
            <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10">
              <div>
                <p className="text-sm font-bold text-white">{course.name}</p>
                <p className="text-[11px] text-slate-400">{course.location}</p>
              </div>
              <button type="button" onClick={() => { setCourse(null); setTee(null); }} className="text-xs text-[#00FF87] font-bold hover:underline flex items-center gap-1">
        ...
        {course && (
          <>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">Your Tee</label>
                ...
                {course.tees.map(t => (
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      tee?.id === t.id ? 'bg-[#05C46B]/20 border-[#00FF87] text-white shadow-md' : 'bg-white/5 border-white/10 text-slate-400 hover:text-white'
                    }`}
                  >
                    <span className="font-bold text-xs">{t.name}</span>
                    <div className="text-[10px] text-slate-400 mt-1">R: {t.rating} / S: {t.slope}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">Holes</label>
                ...
                {([18, 9] as const).map(h => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-extrabold rounded-xl transition-all ${
                      holes === h ? 'bg-gradient-to-r from-[#05C46B] to-[#00FF87] text-[#070B16] shadow-md' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    {h} Holes
                  </button>
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-[#05C46B] to-[#00FF87] hover:opacity-95 text-[#070B16] font-black py-3.5 rounded-xl text-base shadow-lg shadow-[#05C46B]/30 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
