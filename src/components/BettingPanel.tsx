    ...
    <div className="glass-panel p-6 rounded-3xl space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-stone-300 uppercase tracking-wider flex items-center gap-2">
          <HandCoins size={16} className="text-[#C9A24B]" /> Side Bets
        </h2>
        {!showForm && (
          <button
            ...
            className="text-xs text-[#7FA65C] font-bold hover:underline flex items-center gap-1 disabled:text-stone-500 disabled:no-underline disabled:cursor-not-allowed"
          >
      ...
      {showForm && (
        <div className="p-4 rounded-2xl bg-white/5 border border-[#C9A24B]/30 space-y-3">
          <input
            type="text"
          ...
          <div className="relative">
            <DollarSign size={14} className="absolute left-3 top-2.5 text-stone-400" />
            <input
              type="number"
              min="0"
              step="1"
          ...
          <div className="space-y-1.5">
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Who's in?</p>
              ...
              {otherPlayers.map(p => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-all ${
                    selectedParticipants.includes(p.golferId) ? 'bg-[#C9A24B]/20 border-[#C9A24B] text-[#C9A24B]' : 'bg-white/5 border-white/10 text-stone-400'
                  }`}
                >
                  {p.golferName}
                </button>
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="flex-1 text-xs text-stone-400 hover:text-white font-bold py-2">Cancel</button>
            <button
              onClick={handlePropose}
              disabled={!description.trim() || selectedParticipants.length === 0 || submitting}
              className="flex-1 bg-[#C9A24B] text-[#171911] font-bold py-2 rounded-xl text-xs disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
            >
      ...
      {game.bets.length === 0 && !showForm && (
        <p className="text-xs text-stone-400 text-center py-2">No bets yet — bets are optional, propose one if your group's into it.</p>
      )}
          ...
          <div key={bet.id} className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-2">
            <div className="flex items-start justify-between gap-2">
              ...
              {bet.createdBy === currentUser.id && (
                <button onClick={() => handleCancel(bet.id)} disabled={busyBetId === bet.id} className="text-stone-500 hover:text-red-400 shrink-0">
                  <X size={14} />
                </button>
              )}
            </div>
              ...
              {bet.participants.map(p => (
                <span
                  ...
                  className={`text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 ${
                    p.agreed ? 'bg-[#7FA65C]/15 text-[#7FA65C]' : 'bg-white/5 text-stone-400'
                  }`}
                >
            ...
            <div className="flex items-center gap-2 pt-1">
              {me && !me.agreed && (
                <button
                  ...
                  className="text-xs bg-[#7FA65C]/20 text-[#7FA65C] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 disabled:opacity-50"
                >
            ...
            {settlingBetId === bet.id && (
              <div className="p-2.5 rounded-xl bg-[#C9A24B]/10 border border-[#C9A24B]/30 space-y-2">
                <p className="text-[10px] text-stone-300">Who won? (optional)</p>
                <div className="flex flex-wrap gap-1.5">
                  {bet.participants.map(p => (
                    <button
                      key={p.golferId}
                      ...
                      disabled={busyBetId === bet.id}
                      className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/10 text-white hover:bg-[#C9A24B]/20 hover:text-[#C9A24B] disabled:opacity-50"
                    >
                      {p.golferName}
                    </button>
                  ))}
                  <button
                    ...
                    disabled={busyBetId === bet.id}
                    className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/10 text-stone-300 disabled:opacity-50"
                  >
                    Skip
                  </button>
                </div>
      ...
      {settledBets.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-white/5">
          <p className="text-[10px] font-bold text-stone-500 uppercase tracking-wider">Settled</p>
          {settledBets.map(bet => (
            <div key={bet.id} className="flex items-center justify-between p-2.5 rounded-xl bg-white/5 border border-white/5">
              <p className="text-xs text-stone-300">{bet.description}</p>
              <p className="text-[10px] text-stone-500">
