...
import { X, Copy, Check, QrCode, UserPlus, Share2, Sparkles } from 'lucide-react';
...
export const QrShareModal: React.FC<QrShareModalProps> = ({
  ...
  const handleAddFriendSubmit = (e: React.FormEvent) => {
    ...
    const res = addFriendByCode(friendCodeInput);
    if (res.success) {
  ...
  return (
    <div className="modal-overlay">
      <div className="glass-panel w-full max-w-md p-6 sm:p-8 rounded-3xl border-white/15 relative space-y-6 text-center">
        ...
        <form onSubmit={handleAddFriendSubmit} className="space-y-3 border-t border-white/10 pt-4 text-left">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#0E1626] border border-white/15 rounded-xl px-4 py-2.5 text-white text-sm font-mono font-bold uppercase focus:outline-none focus:border-[#00FF87]"
            />
            <button
              type="submit"
              className="bg-gradient-to-r from-[#05C46B] to-[#00FF87] text-[#070B16] font-extrabold px-4 py-2.5 rounded-xl text-xs"
            >
              Connect
            </button>
          </div>
